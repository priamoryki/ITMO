...
int main(int argc, char *args[]) {
	...
	while (--len >= 0) {
		fprintf(input, "Reading at malicious_x = %p... ", (void *)malicious_x);
		readMemoryByte(malicious_x++, value, score);
		fprintf(input, "%s: ", (score[0] >= 2 * score[1] ? "Success" : "Unclear"));
		fprintf(input, "0x%02X='%c' score=%d ", value[0],
		       (value[0] > 31 && value[0] < 127 ? value[0] : '?'), score[0]);
		if (score[1] > 0)
			fprintf(input, "(second best: 0x%02X='%c' score=%d)", value[1],
