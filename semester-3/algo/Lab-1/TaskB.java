...
public class TaskB {
    private static int n, m, timer, bridgeAmount;
    ...
    private static HashMap<Pair, Boolean> result = new HashMap<>();
    ...
    private static void dfs(int v, int u) {
        ...
        for (int to : out[v]) {
            ...
            if (used[to])
                ...
                if (up[to] > timeIn[v]) {
                    bridgeAmount++;
                    result.put(new Pair(v, to), true);
                    result.put(new Pair(to, v), true);
                }
            }
        }
    }
    ...
    public static void main(String[] args) {
        ...
        findBridges();
        writer.write(bridgeAmount + "\n");
        for (int num = 0; num < m; ++num) {
            Pair edge = edges.get(num);
            if (result.getOrDefault(edge, false)) {
                writer.write((num + 1) + " ");
            }
        }
        writer.close();
