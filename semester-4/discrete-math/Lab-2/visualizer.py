...
try:
    ...
    while (True):
        ...
        for i in range(tapes_number):
            ...
            stdscr.addstr(line_cnt + 2, carriage_shift - (len(str(carriages_position[i])) - 1) // 2,
                          str(carriages_position[i]))
        ...
        if not (cur, cur_symbols) in graph:
            if tapes_number == 1:
                outcome = colorama.Fore.RED + "Failed, No edge from %s by symbol %s, Rejected" % (
                    cur, cur_symbols[0]) + colorama.Style.RESET_ALL
            else:
                outcome = colorama.Fore.RED + "Failed, No edge from %s by symbols (%s), Rejected" % (
                    cur, ", ".join(cur_symbols)) + colorama.Style.RESET_ALL
            cur = reject
            break
